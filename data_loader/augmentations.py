...
class Compose(object):
    ...
    def __call__(self, img, mask, boundary):
        if isinstance(img, np.ndarray):
            ...
            boundary = Image.fromarray(boundary, mode="L")

            self.PIL2Numpy = True

        assert img.size == mask.size
        for a in self.augmentations:
            img, mask, boundary = a(img, mask, boundary)

        if self.PIL2Numpy:
            img, mask, boundary = np.array(img), np.array(mask, dtype=np.uint8), np.array(boundary, dtype=np.uint8)
...
class AdjustGamma(object):
    ...
    def __call__(self, img, mask, boundary):
        assert img.size == mask.size
        return tf.adjust_gamma(img, random.uniform(1, 1 + self.gamma)), mask, boundary
...
class AdjustContrast(object):
    ...
    def __call__(self, img, mask, boundary):
        assert img.size == mask.size
        return tf.adjust_contrast(img, random.uniform(1 - self.cf, 1 + self.cf)), mask, boundary
    ...
    def __call__(self, img, mask, boundary):
        if random.random() < self.p:
            return (img.transpose(Image.FLIP_LEFT_RIGHT), mask.transpose(Image.FLIP_LEFT_RIGHT),
                    boundary.transpose(Image.FLIP_LEFT_RIGHT))
        return img, mask, boundary
    ...
    def __call__(self, img, mask, boundary):
        if random.random() < self.p:
            return (img.transpose(Image.FLIP_TOP_BOTTOM), mask.transpose(Image.FLIP_TOP_BOTTOM),
                    boundary.transpose(Image.FLIP_TOP_BOTTOM))
        return img, mask, boundary
