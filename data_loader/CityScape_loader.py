...
class CityscapesLoader(data.Dataset):
    ...
    def __init__(self, root, split="train", gt="gtFine", img_size=(1024, 2048),
                 is_transform=False, augmentations=None, is_get_edge=False):
        """
    ...
    def __getitem__(self, index):
        ...
        img = misc.imresize(img, (int(img.shape[0] / 2), int(img.shape[1] / 2), "bilinear"))
        ...
        lbl = misc.imresize(lbl, (int(lbl.shape[0] / 2), int(lbl.shape[1] / 2)), "nearest", mode='F')
        lbl = self.encode_segmap(np.array(lbl, dtype=np.uint8))
        edge_map = self.get_edge(lbl)
    ...
    def get_edge(self, lbl):
        reserved_label_id = [7, 22, 20, 26]
        for i, label in enumerate(reserved_label_id):
            lbl[lbl == label] = i * 50
        for i in range(34):
            if i not in reserved_label_id:
                lbl[lbl == i] = 0
        img, contours, hier = cv2.findContours(lbl, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        convas = np.zeros(lbl.shape, dtype=np.uint8)
        cv2.drawContours(convas, contours, -1, 1, 3)
        return convas
